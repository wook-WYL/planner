...
void FARMaster::MainLoopCallBack()
{
  ...
  if (is_reset_env_)
  {
    ...
    is_reset_env_ = false;
    if (FARUtil::IsDebug)
      RCLCPP_WARN(nh_->get_logger(), "****************** Graph and Env Reset ******************");
    return;
  }
...
{
  if (nav_node_ptr->free_direct != NodeFreeDirect::CONVEX || obsCloudIn->empty())
...
void FARMaster::OdomCallBack(const nav_msgs::msg::Odometry::SharedPtr msg)
{
  ...
  {
    if (FARUtil::IsDebug)
      RCLCPP_WARN_ONCE(nh_->get_logger(), "FARMaster: odom frame does NOT match with world frame!");
...
void FARMaster::PrcocessCloud(const sensor_msgs::msg::PointCloud2::SharedPtr pc, const PointCloudPtr &cloudOut)
{
  ...
  {
    if (FARUtil::IsDebug)
      RCLCPP_WARN_ONCE(nh_->get_logger(), "FARMaster: cloud frame does NOT match with world frame!");
    try
    {
...
void FARMaster::TerrainCallBack(const sensor_msgs::msg::PointCloud2::SharedPtr pc)
{
  ...
  if (!master_params_.is_static_env && !is_stop_update_)
  {
    ...
    {
      if (FARUtil::IsDebug)
        RCLCPP_WARN(nh_->get_logger(), "FARMaster: dynamic obstacle detected, size: %ld", FARUtil::cur_dyobs_cloud_->size());
...
void FARMaster::WaypointCallBack(const geometry_msgs::msg::PointStamped &route_goal)
{
  ...
  {
    if (FARUtil::IsDebug)
      RCLCPP_WARN(nh_->get_logger(), "FARMaster: wait for v-graph to init before sending any goals");
    return;
  }
  ...
  {
    if (FARUtil::IsDebug)
      RCLCPP_WARN_ONCE(nh_->get_logger(), "FARMaster: waypoint published is not on world frame!");
